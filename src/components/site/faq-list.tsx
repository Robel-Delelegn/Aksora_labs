...
export function FaqList({ items, tone = "light" }: FaqListProps) {
  ...
  const rowClassName =
    tone === "dark"
      ? "group border-b border-white/10 py-6 last:border-b-white/14 sm:py-7"
      : "group border-b border-[var(--border)] py-6 last:border-b-[var(--border-strong)] sm:py-7";
  ...
  const iconClassName =
    tone === "dark" ? "mt-1 h-5 w-5 shrink-0 text-white/54 transition group-open:rotate-180" : "mt-1 h-5 w-5 shrink-0 text-slate-500 transition group-open:rotate-180";
